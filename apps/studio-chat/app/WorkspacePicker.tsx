...
/* ─── Dev Mode Handoff Dialog ─── */

function DevModeHandoff({
  workspace,
  onClose,
}: {
  ...
  return (
    <>
      ...
      <div className="fixed top-16 left-1/2 -translate-x-1/2 z-50 w-full max-w-lg rounded-2xl border border-edge bg-elevated shadow-2xl overflow-hidden scale-in max-h-[80dvh] flex flex-col">
        ...
        {/* Actions bar — two clear paths */}
        <div className="flex items-center gap-2 px-4 pt-3 pb-2 shrink-0">
          <button
            onClick={() => setShowNewDialog(true)}
            className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-[12px] font-medium text-white bg-accent rounded-xl hover:brightness-110 transition-all"
          >
            <PlusIcon />
            New Project
          </button>
          <div className="flex-1 relative">
            <input
              ...
              placeholder="Add existing folder…"
              className="w-full rounded-xl border border-edge bg-surface pl-3 pr-9 py-2 text-[12px] font-mono outline-none focus:border-accent/40 placeholder:text-ink-faint transition-all"
            />
            <button
              onClick={handleAdd}
              disabled={!addPath.trim() || isAdding}
              className="absolute right-1 top-1/2 -translate-y-1/2 size-7 flex items-center justify-center rounded-lg text-ink-faint hover:text-ink hover:bg-deep disabled:opacity-30 transition-all"
              title="Add folder"
            >
              {isAdding ? <span className="size-3.5 rounded-full border-2 border-accent/30 border-t-accent animate-spin" /> : <span className="text-sm font-medium">+</span>}
            </button>
          </div>
        </div>

        {/* Errors */}
        {(switchError || addError) && (
          <div className="px-4 pb-1 shrink-0">
            {switchError && (
              <div className="px-3 py-2 rounded-lg border border-red-200 bg-red-50/50 dark:border-red-900/30 dark:bg-red-950/10 mb-1">
                <p className="text-[11px] text-red-600 dark:text-red-400 whitespace-pre-wrap">{switchError}</p>
              </div>
            )}
            {addError && (
              <div className="px-3 py-2 rounded-lg border border-red-200 bg-red-50/50 dark:border-red-900/30 dark:bg-red-950/10">
                <p className="text-[11px] text-red-600 dark:text-red-400">{addError}</p>
              </div>
            )}
          </div>
        )}
        ...
        {/* Workspace list */}
        <div className="overflow-y-auto flex-1 px-2 py-1 min-h-0">
          {workspaces.length === 0 ? (
            <div className="px-3 py-10 text-center">
              <div className="flex justify-center mb-3 text-ink-faint/40"><FolderOpenIcon /></div>
              <p className="text-[13px] font-medium text-ink-dim">No workspaces yet</p>
              <p className="text-[11px] text-ink-faint mt-1 max-w-xs mx-auto leading-relaxed">
                Create a new project above, or paste a folder path to add an existing one.
              </p>
            </div>
          ) : filtered.length === 0 ? (
            <div className="px-3 py-8 text-center">
              <p className="text-[13px] text-ink-dim">No workspaces match your search</p>
            </div>
          ) : (
      ...
      {handoffWorkspace && (
        <DevModeHandoff
          ...
          onClose={() => { setHandoffWorkspace(null); onClose(); }}
        />
      )}
    </>
  );
